...
| [**novel-script**](skills/novel-script/README.en.md) | Screenwriting for AI short drama: scenes + beat flow, deterministic duration, and a dramatic state chain computed from one scene entry state plus an explicit `statePatch` on every action beat. All 11 quality gates script-checked |
| [**novel-storyboard**](skills/novel-storyboard/README.en.md) | Storyboarding for AI short drama: new seeds use 5–10s segments → 2–5s cuts → one keyframe per cut, route one frame to I2VA and multiple references to Ref2VA, and optionally apply one of eight H3 style presets. Every cut inherits script state before camera direction and continuity. All 23 quality gates script-checked |
