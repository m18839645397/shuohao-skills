...
`cinematic` 与 `naturalistic` 的粗图仍必须是真人摄影选镜联系表：真人演员、实体服装、真实地点/搭景和光学镜头语义。粗糙指细节少，不代表可以变成手绘分镜、概念图或动画草稿。

`naturalistic` 九格即便很粗，也要每格只有一个可读焦平面，非主体平面逐渐变软；不要用“全图等清晰 + 均匀纹理噪声”冒充信息丰富。九格负责选构图，所以只保留会影响人物关系、动作阶段、银幕方向和道具归属的局部材质线索。

九格也必须隔离参考图职责：角色图只供身份、体型、发型和服装，不复制试镜姿势/手势/手持物；场景图不复制原机位；道具图不复制白底展示角度。人物要像被观察到正在处理事情，每只手有功能，视线有对象，道具按真实使用方向转动；不得九格都变成正面排队或向镜头展示线索。
...
第一张必须来自 G1–G3，最后一张必须来自 G7–G9，选择顺序只能从 entry 向 result 推进。entry 只早于本段新动作；上个状态已存在的搬运、受力、接触或环境动势仍要保持当前阶段，不能回到静物陈列状态。
