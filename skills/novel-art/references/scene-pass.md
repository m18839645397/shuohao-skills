...
5. **风格整套取用。**跑 `node scripts/novel-art.mjs styles` 拿当前风格的 render/surface/negative/tags，`image.sheet` 里整段带上渲染句和表面处理句，不要混搭两种风格。
   ...
   `naturalistic` 使用现实存在的普通地点/物件、可用日光或可信实用灯、非设计化杂乱和中性色。先指定一个真正的焦平面，前后层次自然变软；指纹集中在把手/开关，通行磨损落在路径，水渍位于渗漏下方，灰尘停在遮蔽角落，不能把污渍和微纹理均匀铺满全景。不得加入全图等清晰、假景深、广告式主体分离、戏剧雾、英雄构图、奢华置景或大片式青橙调色。
...
7. **不要把角色 skill 的表面处理带进来。**毛孔、皮下散射是皮肤的事；环境的真实感来自**有原因的用旧**——掉漆、水渍、包浆和磨白木纹必须落在会被触碰、踩踏、渗漏或修补的位置。物件还要有重量、支撑压缩和接地阴影。预设里写好了，照用。
