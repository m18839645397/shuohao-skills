...
5. `image.prompt` 是**单张表现性身份图**（不是技术三视图）：四分之三视角半身、纯中性背景、方向主光、浅景深、面部最实。渲染方式必须从本次 `style` 预设整块取用，不能硬编码默认 realistic。
   ...
   - `naturalistic` 必须逐字使用预设的 `Naturalistic live-action documentary casting photography...`：焦点平面最清楚，远侧五官和后发际自然变软；皮肤、发丝、服装磨损按部位/使用原因局部分配；站姿有承重偏移，接触有压缩、变色或位移；使用不完美现场光，并禁止均匀微细节、全图等清晰、假景深、完美影棚分离、戏剧轮廓光和英雄姿态
   - `ghibli` / `inkwash` 使用各自完整预设，不混入摄影皮肤和光学镜头词

   **真实感来自信息分配和物理因果，不是细节量。** 皮肤和五官要写具体，但不能把毛孔、虹膜、发丝都平均锐化：鼻与内脸颊毛孔较强，眼睑和上脸颊更平滑，额头可有不规则油光，鼻孔/耳缘可有局部泛红；近眼或主面部平面最清楚，远眼、后脸颊、耳朵和后发际自然变软。**眼睑和眉毛左右略不对称**；只有少量碎发/发束清楚，其余头发允许并入暗部块面。老年角色收益最大：老年斑、皮肤松弛，**皱纹要顺着表情肌走**，不是随机刻线。

   **布料决定「像不像真衣服」**：织纹不能全身等强，磨损只落在袖口、肘膝、领口、背带压痕等真实使用位置；布料垂坠有重量，褶皱与接触点产生压缩、自阴影和位移。
